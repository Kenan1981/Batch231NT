...
public class E01 {
    public static void main(String[] args) {
        ...
        bolme(5, 0);//Hatanın sebebi 0 gönderilmesi. satır numaralarını hata da belirtiyor.

        bolme2(7, 2);//HATA try-catch ile hata mesajı yazıldı
    ...
    public static void bolme(int a, int b) {
        if (b == 0) {
            System.out.println("Lütfen Sıfır Girmeyiniz");//Hata yerine "Lütfen Sıfır Girmeyiniz" yazdı. if le kontrol edildi.
        } else {
    ...
    public static void bolme2(int a, int b) {
        try {
            System.out.println(a / b);//3 hata almadığı için çalıştı
            System.out.println("try için görev tamamlandı");

        } catch (ArithmeticException e) {
            System.out.println("Lütfen Sıfıra Bölme İşlemi Yapmayınız");
        }
